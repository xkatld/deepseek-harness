...
import {
  ...
  DETAILS_COLLAPSED, DETAILS_DEFAULT, DETAILS_MIN, SIDEBAR_COLLAPSED, SIDEBAR_DEFAULT, SIDEBAR_MIN,
...
describe('computeColumns', () => {
  ...
  it('collapsed sidebars keep both compact rails', () => {
    expect(computeColumns(1920, closed(300), closed(360)))
      .toEqual({ sidebar: SIDEBAR_COLLAPSED, center: 1920 - SIDEBAR_COLLAPSED - DETAILS_COLLAPSED, details: DETAILS_COLLAPSED })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    ...
    expect(cols).toEqual({ sidebar: 280, center: 930 - DETAILS_COLLAPSED, details: DETAILS_COLLAPSED })
  })
  ...
  it('the sidebar never concedes: center absorbs the deficit below CENTER_MIN', () => {
    ...
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 420 - DETAILS_COLLAPSED, details: DETAILS_COLLAPSED })
  })
  ...
  it('sidebar-closed narrow window: details concedes then auto-closes', () => {
    ...
    expect(starved).toEqual({
      ...
      center: DETAILS_MIN + CENTER_MIN - 1 - DETAILS_COLLAPSED,
      details: DETAILS_COLLAPSED,
    })
  })
  ...
  it('tiny viewport: details closes, sidebar holds, center takes the remainder', () => {
    ...
    expect(cols.details).toBe(DETAILS_COLLAPSED)
    expect(cols.sidebar).toBe(SIDEBAR_DEFAULT)
    expect(cols.center).toBe(Math.max(0, 400 - SIDEBAR_DEFAULT - DETAILS_COLLAPSED))
  })
  ...
  it('recovery is pure: re-widening restores preferred widths untouched', () => {
    ...
    expect(squeezed.details).toBe(DETAILS_COLLAPSED)
...
describe('computeColumns — degenerate viewports', () => {
  it('sidebar closed and viewport below CENTER_MIN: details auto-closes, center takes the rest', () => {
    ...
    expect(computeColumns(500, closed(300), open(DETAILS_DEFAULT)))
      .toEqual({ sidebar: SIDEBAR_COLLAPSED, center: 500 - SIDEBAR_COLLAPSED - DETAILS_COLLAPSED, details: DETAILS_COLLAPSED })
  })
})
